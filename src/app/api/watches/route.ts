...
export async function POST(request: NextRequest) {
  ...
  // Deactivate + cancel any existing Campflare subscription for this campground+dates
  ...
  if (existing) {
    ...
    await query(`UPDATE watches SET active = false WHERE id = $1`, [existing.id]);
  }

  // Create the DB watch first so we have its ID for the Campflare metadata
  ...
  // Subscribe to Campflare (best-effort — watch is still valid without it)
  ...
  if (webhookBase && process.env.CAMPFLARE_API_KEY) {
    try {
      ...
      await query(
        `UPDATE watches SET campflare_sub_id = $1 WHERE id = $2`,
        [sub.id, row.id]
      );
    ...
    if (!process.env.CAMPFLARE_API_KEY) {
      console.log('[watches] CAMPFLARE_API_KEY not set — watch saved, Campflare skipped');
    } else {
      console.log('[watches] No webhook URL configured (set NEXT_PUBLIC_APP_URL) — watch saved, Campflare skipped');
    }
  }
...
export async function DELETE(request: NextRequest) {
  ...
  // Cancel the Campflare subscription if one exists
