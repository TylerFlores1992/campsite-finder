...
REM SAY WHAT WE LANDED ON. auto-update.ps1 reports through Report-Applied; this manual
REM path never did, so the admin panel kept showing whatever the last UNATTENDED run had
REM recorded - on 2026-08-11 it read "37e1527, REFUSED - processes would not stop" while
REM this box was happily running d1ab782. A record that is confidently wrong is worse than
REM none, because it is exactly the field you check to find out whether a fix arrived.
REM
REM Never fatal: the update has already happened by this point.
node "%~dp0report-applied.mjs" "manual update.bat"
